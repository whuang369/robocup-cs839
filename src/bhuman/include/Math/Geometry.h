...
STREAMABLE(Circle, {
  ...
  Circle(const Vector2f& c, float r),
      ...
      (Vector2f)(Vector2f::Zero())center, (float)(0)radius,
});

inline Circle::Circle(const Vector2f& c, float r) : center(c), radius(r) {}
...
STREAMABLE(Rect, {
  ...
  Rect(const Vector2f& a, const Vector2f& b),
      ...
      (Vector2f)(Vector2f::Zero())a, (Vector2f)(Vector2f::Zero())b,
});

inline Rect::Rect(const Vector2f& a, const Vector2f& b) : a(a), b(b) {}
...
STREAMABLE(Line, {
  ...
  void normalizeDirection(),
      ...
      (Vector2f)base, (Vector2f)direction,
});
 ...
 */
int getIntersectionOfLineAndCircle(const Line& line, const Circle& circle,
                                   Vector2f& firstIntersection, Vector2f& secondIntersection);
bool checkIntersectionOfLines(const Vector2f& l1p1, const Vector2f& l1p2, const Vector2f& l2p1,
                              const Vector2f& l2p2);
[[nodiscard]] bool getIntersectionOfLines(const Line& line1, const Line& line2,
                                          Vector2f& intersection);
[[nodiscard]] bool getIntersectionOfRaysFactor(const Line& ray1, const Line& ray2,
                                               float& intersection);
[[nodiscard]] bool getIntersectionOfLineAndConvexPolygon(const std::vector<Vector2f>& polygon,
                                                         const Line& direction,
                                                         Vector2f& intersection, const bool isCCW,
                                                         Line* intersectedLine = nullptr);

/**
 * Computes the signed distance of a point to a line.
 * Be careful: This means that NEGATIVE distances might occur. If you need the absolute distance,
 * put an abs() around the result.
 * @param line The line.
 * @param point The point
 * @return The signed distance between point and line. BEWARE, it might be negative, depending on
 * which side of the line the point is!
 */
...
[[nodiscard]] bool isPointInsideRectangle(const Vector2f& bottomLeftCorner,
                                          const Vector2f& topRightCorner, const Vector2f& point);
[[nodiscard]] bool isPointInsideRectangle(const Vector2i& bottomLeftCorner,
                                          const Vector2i& topRightCorner, const Vector2i& point);
[[nodiscard]] bool isPointInsideRectangle2(const Vector2f& corner1, const Vector2f& corner2,
                                           const Vector2f& point);
[[nodiscard]] bool isPointInsideRectangle(const Rect& rect, const Vector2f& point);
[[nodiscard]] bool isPointInsideConvexPolygon(const Vector2f polygon[], const int numberOfPoints,
                                              const Vector2f& point);
...
[[nodiscard]] bool isPointInsideArc(const Vector2f& point, const Vector2f& center,
                                    const Rangea& angleRange, const float radius);
bool clipPointInsideRectangle(const Vector2i& bottomLeftCorner, const Vector2i& topRightCorner,
                              Vector2i& point);
bool clipPointInsideRectangle(const Vector2i& bottomLeftCorner, const Vector2i& topRightCorner,
                              Vector2f& point);
...
/**
 * Checks, if a circle intesects with a rectangle. The rectangle is assumed to be axis-aligned and
 * defined by two points, which have to be diagonally opposing corners.
 ...
 */
[[nodiscard]] bool circleIntersectsAxisAlignedRectangle(const Vector2f& cp, float r,
 ...
 * @return states whether clipping was necessary (and done)
 */
[[nodiscard]] bool getIntersectionPointsOfLineAndRectangle(const Vector2i& bottomLeft,
                                                           const Vector2i& topRight,
                                                           const Geometry::Line& line,
                                                           Vector2i& point1, Vector2i& point2);
[[nodiscard]] bool getIntersectionPointsOfLineAndRectangle(const Vector2f& bottomLeft,
 ...
 */
Vector2f getOrthogonalProjectionOfPointOnLine(const Vector2f& base, const Vector2f& dir,
                                              const Vector2f& point);
 ...
 */
Vector2f getOrthogonalProjectionOfPointOnEdge(const Vector2f& base, const Vector2f& dir,
                                              const Vector2f& point);
