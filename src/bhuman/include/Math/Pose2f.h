...
STREAMABLE(Pose2f, {
  ...
  /**
   * Difference of this pose relative to another pose. So if A+B=C is the addition/concatenation,
  ...
  static Pose2f random(const Rangef& angle, const Rangef& x, const Rangef& y),
      ...
      (Angle)(0)rotation, (Vector2f)(Vector2f::Zero())translation,
});
...
inline Pose2f& Pose2f::invert() {
  ...
  const float s = std::sin(rotation);
  const float c = std::cos(rotation);
  translation = Vector2f(c * trans.x() - s * trans.y(), s * trans.x() + c * trans.y());
  return *this;
}
...
inline Pose2f Pose2f::random(const Rangef& angle, const Rangef& x, const Rangef& y) {
  return Pose2f(Random::uniform(angle.min, angle.max),
                Vector2f(Random::uniform(x.min, x.max), Random::uniform(y.min, y.max)));
}
