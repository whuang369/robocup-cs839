...
class UKFPose2D {
 ...
 public:
   ...
   * @return The pose
   */
  Pose2f getPose() const { return Pose2f(mean.z(), mean.head<2>()); }

  /** Returns a reference to the internal representation of the state's covariance matrix
   * @return A reference to the matrix
   */
  const Matrix3f& getCov() const { return cov; }

  /** Pose update based on the assumed robot motion
   * @param odometryOffset The pos(e)itional changes regarding translation and rotation (as reported
   * by motion modules)
   ...
   * @param odometryRotationDeviation Additional odometry uncertainty of rotation that affects
   * translation
   */
 ...
 protected:
   ...
   */
  void landmarkSensorUpdate(const Vector2f& landmarkPosition, const Vector2f& reading,
                            const Matrix2f& readingCov);

  /** Pose update based on the measurement of a perceived line
   * @param lineIsParallelToWorldModelXAxis Flag. Set this to "true", if the perceived line is
   * assumed to be parallel to the field's x-axis (given a previous registration process). In this
   * case, only the robot's y-coordinate as well as its rotation can be updated. If this flag is set
   * to false, only the robot's x-coordinate as well as its rotation can be updated, as the line is
   * assumed to be parallel the field's halfway line (y-axis). We assume that there are no major
   * diagonal lines on a football pitch.
   * @param reading The first element of this vector is a measured coordinate on the field.
   *                If lineIsParallelToWorldModelXAxis is set to true, it is a y-coordinate, an
   * x-coordinate otherwise. The second component is always a measured rotation.
   * @param readingCov The covariance of the measurement/reading
   */
  void lineSensorUpdate(bool lineIsParallelToWorldModelXAxis, const Vector2f& reading,
                        const Matrix2f& readingCov);
   ...
   * @param readingCov The covariance of the measurement (based on the relative measurement of some
   * features indicating the absolute pose)
   */
