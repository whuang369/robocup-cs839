...
class FunctionList {
  ...
  struct Element {
     ...
     */
    Element(void (*const function)()) : function(function), next(first) { first = this; }
  };
 ...
 public:
  ...
  class Registrar {
   ...
   public:
    ...
    Registrar() { static_cast<void>(&Registrar::element); }
  };
...
template <void (*const function)()>
FunctionList::Element FunctionList::Registrar<function>::element = FunctionList::Element(function);
 ...
 * that case, the PUBLISH statement must be inside a method that will definitely be
 * instantiated.
 */
#define PUBLISH(...) \
  { static FunctionList::Registrar<__VA_ARGS__> publish; }
