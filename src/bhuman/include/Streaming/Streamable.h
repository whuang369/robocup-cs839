 ...
 * @param class The class that is registered. It is not allowed to contain commas (use COMMA
 * instead).
...
#define _REG_II_1(attribute) \
  TypeRegistry::addAttribute(_type, typeid(decltype(_class->attribute)).name(), #attribute)
...
#define _REG_II_10(a0, a1, a2, a3, a4, a5, a6, a7, a8, a9) \
  _REG_III(a9, a0, a1, a2, a3, a4, a5, a6, a7, a8)
#define _REG_III(attribute, ...) \
  TypeRegistry::addAttribute(_type, typeid(__VA_ARGS__).name(), #attribute)
...
template <typename T>
In& streamStaticArray(In& in, T inArray[], size_t size, const char* enumType) {
  return streamComplexStaticArray(in, inArray, size, enumType);
}
template <typename T>
Out& streamStaticArray(Out& out, T outArray[], size_t size, const char* enumType) {
  return streamComplexStaticArray(out, outArray, size, enumType);
}

template <typename T, typename U>
void cast(T& t, const U& u) {
  t = static_cast<T>(u);
}
...
struct Streamer<std::vector<E, A>> {
  ...
  static void read(In& stream, const char* name, S& s) {
    ...
    stream >> size;
    s.resize(size);
    if (!s.empty()) streamStaticArray(stream, &s[0], s.size() * sizeof(E), enumType);
    stream.deselect();
  }
...
template <typename E>
struct Streamer<std::optional<E>> {
...
template <typename S>
void streamIt(In& stream, const char* name, S& s) {
  Streamer<S>::read(stream, name, s);
}

/** Write variable to a stream. */
template <typename S>
void streamIt(Out& stream, const char* name, const S& s) {
  Streamer<S>::write(stream, name, const_cast<S&>(s));
}

/** This is the version for using inside operator>>. */
template <typename S>
void streamItExt(In& stream, const char* name, S& s) {
  streamIt(stream, skipDot(name), s);
}

/** This is the version for using inside operator<<. */
template <typename S>
void streamItExt(Out& stream, const char* name, const S& s) {
  streamIt(stream, skipDot(name), s);
}
