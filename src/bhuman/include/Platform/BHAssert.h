...
#define ASSERT(cond)                                                                             \
  static_cast<void>((cond) ? 0                                                                   \
                           : (Assert::logAdd(0, __FILE__, __LINE__, "ASSERT(" #cond ") failed"), \
                              Assert::abort(), 0))
#else
#define ASSERT(cond)                                                                         \
  static_cast<void>((cond) ? 0                                                               \
                           : (Assert::print(__FILE__, __LINE__, "ASSERT(%s) failed", #cond), \
                              Assert::abort(), 0))
#endif
...
#define VERIFY(cond)                                                                             \
  static_cast<void>((cond) ? 0                                                                   \
                           : (Assert::logAdd(0, __FILE__, __LINE__, "VERIFY(" #cond ") failed"), \
                              Assert::abort(), 0))
#else
#define VERIFY(cond)                                                                         \
  static_cast<void>((cond) ? 0                                                               \
                           : (Assert::print(__FILE__, __LINE__, "VERIFY(%s) failed", #cond), \
                              Assert::abort(), 0))
#endif
...
/**
 * Adds a warning to the output of the compiler:
 * Example: WARNING("Replace this later")
 */
#define _WARNING(text) #text
#ifdef WINDOWS
#define _WARNING2(text) _WARNING(text)
#define WARNING(text) \
  _Pragma(_WARNING(message(__FILE__ "(" _WARNING2(__LINE__) "): warning: " text)))
#else
#define WARNING(text) _Pragma(_WARNING(GCC warning text))
#endif
