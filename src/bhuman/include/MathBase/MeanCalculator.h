...
class MeanCalculator {
 ...
 public:
  MeanCalculator(const CallbackType& callback = MeanCalculator<ResultType, ValueType>::id)
      : sum(ZeroType<ResultType>::value()), count(0), callback(callback) {}
  ...
  operator ResultType() const { return ResultType(sum / count); }
};
...
static inline float calcMeanSquared(const IteratorType& begin, const IteratorType& end) {
  return std::sqrt(static_cast<float>(MeanCalculator<ValueType, ValueType>([](const ValueType& v) {
                                        return v * v;
                                      }).add(begin, end)));
}
...
static inline ResultType calcMeanSquared(const std::array<ValueType, N>& data) {
  return std::sqrt(static_cast<float>(
      MeanCalculator<ValueType, ValueType>([](const ValueType& v) { return v * v; }).add(data)));
}
