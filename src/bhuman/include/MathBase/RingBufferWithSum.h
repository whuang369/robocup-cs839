...
class RingBufferWithSum : public RingBuffer<T, n> {
 private:
 ...
 public:
  ...
  RingBufferWithSum(std::size_t capacity = n) : RingBuffer<T, n>(capacity), zero() {
    static_assert(std::is_same<Angle, T>::value || std::is_floating_point<T>::value ||
                      std::is_integral<T>::value,
                  "The standard constructor is not applicable for non-standard types in the "
                  "template argument");
    clear();
  }
  ...
  RingBufferWithSum(const T& zero, std::size_t capacity = n)
      : RingBuffer<T, n>(capacity), zero(zero) {
    static_assert(!std::is_same<Angle, T>::value && !std::is_floating_point<T>::value &&
                      !std::is_integral<T>::value,
  ...
  void reserve(std::size_t capacity) {
    while (RingBuffer<T, n>::size() > capacity) pop_back();
  ...
  void push_front(const T& value) {
    if (RingBuffer<T, n>::full()) prevSum -= RingBuffer<T, n>::back();
...
T RingBufferWithSum<T, n>::average() const {
  if (RingBuffer<T, n>::empty())
    return zero;
  else
...
template <typename T, std::size_t n>
float RingBufferWithSum<T, n>::averagef() const {
