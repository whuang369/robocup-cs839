...
class LineFitter : public MeanCalculator<Eigen::Matrix<float, 2, 3>, Vector2f> {
 public:
  LineFitter()
      : MeanCalculator<Eigen::Matrix<float, 2, 3>, Vector2f>([](const Vector2f& v) {
          Eigen::Matrix<float, 2, 3> m;
          m.col(0) = v;
          m.col(1) = v.cwiseProduct(v);
          m(0, 2) = v.x() * v.y();
          return m;
        } /*lambda ends here*/) {}  // empty body
...
class CircleFitter : public MeanCalculator<Eigen::Matrix<float, 2, 4>, Vector2f> {
 public:
  CircleFitter()
      : MeanCalculator<Eigen::Matrix<float, 2, 4>, Vector2f>([](const Vector2f& v) {
...
static inline bool fitLine(const IteratorType& begin, const IteratorType& end, Vector2f& n0,
                           float& d) {
  LineFitter fitter;
 ...
 */
template <typename IteratorType>
static inline bool fitCircle(const IteratorType& begin, const IteratorType& end, Vector2f& center,
                             float& radius) {
  CircleFitter fitter;
  fitter.add(begin, end);
