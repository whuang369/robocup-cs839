...
class RingBuffer {
 ...
 public:
  ...
  class iterator {
   ...
   public:
    ...
    iterator& operator=(const iterator& other) {
      return *new (this) iterator(other.buffer, other.index);
    }
  ...
  RingBuffer(const RingBuffer& other)
    ...
    for (std::size_t i = other.entries; i-- > 0;) push_front(other[i]);
  }

  /** Destructor. */
  ~RingBuffer() {
    clear();
    if (buffer) Memory::alignedFree(reinterpret_cast<char*>(buffer));
  }
  ...
  void reserve(std::size_t capacity) {
    if (capacity != allocated) {
      ...
      if (prev) Memory::alignedFree(reinterpret_cast<char*>(prev));
    }
  }
