...
class UnscentedKalmanFilter {
 ...
 public:
  ...
  CovarianceType cov =
      CovarianceType::Zero();  // The covariance of the hypothesis to quantify the certainty
 ...
 public:
  ...
  /**
   * The prediction step to propagate the whole hypothesis with a given dynamic model and an
   * operation specific noise. In other works this function is referred as dynamic step.
   ...
   * @param measurementNoise, the measurement specific noise (as a variance) to quantify the
   * uncertainty
   */
  template <unsigned N>
  void update(const Vectorf<N>& measurement,
              const std::function<Vectorf<N>(const State&)>& measurementModel,
              const Eigen::Matrix<float, N, N>& measurementNoise);
   ...
   * uncertainty
   */
  void update(float measurement, const std::function<float(const State&)>& measurementModel,
 ...
 */
template <typename State, unsigned DOF, bool Manifold>
void UnscentedKalmanFilter<State, DOF, Manifold>::init(const State& initState,
                                                       const CovarianceType& initNoise) {
  mean = initState;
  cov = initNoise;
  sigmaPoints.fill(initState);
}
...
/**
 * The prediction step to propagate the whole hypothesis with a given dynamic model and an operation
 * specific noise. In other works this function is referred as dynamic step.
 * @param dynamicModel, a function to propagate the state
 * @param noise, the propagation specific noise (as variance) to quantify the uncertainty
 */
template <typename State, unsigned DOF, bool Manifold>
void UnscentedKalmanFilter<State, DOF, Manifold>::predict(
    const std::function<void(State&)>& dynamicModel, const CovarianceType& noise) {
  ASSERT((noise.array() >= 0.f).all());
  ASSERT(noise.trace() > 0.f);

  updateSigmaPoints();

  for (State& sigmaPoint : sigmaPoints) dynamicModel(sigmaPoint);
...
template <unsigned N>
void UnscentedKalmanFilter<State, DOF, IsManifold>::update(
    const Vectorf<N>& measurement, const std::function<Vectorf<N>(const State&)>& measurementModel,
    const Eigen::Matrix<float, N, N>& measurementNoise) {
  ...
  for (size_t i = 0; i < sigmaPoints.size(); ++i) Z[i] = measurementModel(sigmaPoints[i]);

  MeasurementType z = MeasurementType::Zero();
  for (MeasurementType& Zi : Z) z += Zi;
  ...
  if (sigmaz.determinant() != 0) K = sigmaxz * sigmaz.inverse();
  ASSERT(K.allFinite());
...
template <typename State, unsigned DOF, bool IsManifold>
void UnscentedKalmanFilter<State, DOF, IsManifold>::update(
    float measurement, const std::function<float(const State&)>& measurementModel,
    float measurementNoise) {
  ...
  float sigmaz = 0.f;
  for (float& Zi : Z) sigmaz += sqr(Zi - z);
  sigmaz *= 0.5f;
  sigmaz += measurementNoise;
...
void UnscentedKalmanFilter<State, DOF, IsManifold>::updateSigmaPoints() {
  ...
  if (info == Eigen::ComputationInfo::Success) {
    ...
    sigmaPoints[0] = mean;
    for (unsigned i = 0; i < DOF; ++i) sigmaPoints[i + 1] = mean + l.col(i);
    for (unsigned i = 0; i < DOF; ++i) sigmaPoints[i + DOF + 1] = mean + (-l.col(i));
  } else {
    // maybe adding 1 sigma or something is better?
    for (State& sigmaPoint : sigmaPoints) sigmaPoint = mean;
  }
}
 ...
 */
template <typename State, unsigned DOF, bool IsManifold>
inline void UnscentedKalmanFilter<State, DOF, IsManifold>::covarianceMatrixValidation(
    [[maybe_unused]] const CovarianceType& cov) const {
 ...
 */
template <typename State, unsigned DOF, bool IsManifold>
inline void UnscentedKalmanFilter<State, DOF, IsManifold>::fixCovarianceMatrix(
    CovarianceType& cov) {
  for (unsigned i = 0; i < DOF; ++i) {
    for (unsigned j = i + 1; j < DOF; ++j) {
...
struct MeanOfSigmaPoints<State, DOF, Array, true> {
  ...
  static State run(const Array& sigmaPoints) {
    ...
    // Move the mean each step into the direction of the sigma points till the difference is minimal
    // or the limit is reached
    do {
      lastMean = mean;
      ++iterations;
      Vectorf sum = Vectorf::Zero();
      for (const State& sigmaPoint : sigmaPoints) sum += sigmaPoint - mean;
...
struct MeanOfSigmaPoints<State, DOF, Array, false> {
  ...
  static State run(const Array& sigmaPoints) {
    ...
    for (const State& sigmaPoint : sigmaPoints) sum += sigmaPoint;
