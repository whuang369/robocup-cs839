 ...
 */
inline void errorEllipse(const Matrix2f& m, float& axis1, float& axis2, float& angle,
                         const float p = 0.95f) {
  ...
  angle = 0.f;
  if (m(1, 0) !=
      0.f)  // There must be a covariance. Otherwise, the ellipse is not rotated (== axis-aligned)
  {
...
inline void fixCovariance(Eigen::Matrix<float, SIZE, SIZE>& m) {
  for (int i = 0; i < SIZE; i++) {
    for (int j = i + 1; j < SIZE; j++) m(i, j) = m(j, i) = (m(i, j) + m(j, i)) * 0.5f;
  }
}
}  // namespace Covariance
