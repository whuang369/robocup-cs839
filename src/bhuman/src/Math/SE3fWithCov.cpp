 ...
 * it is important to note that the covariance is in the tangent space of the SE3 lie group (lie
 * algebra) so it can not be directly converted to Cartesian coordinates but instead via the
 * exponential map
 *
 * Note that the uncertainty is applied in the local reference frame
 *
 * [1] L. Meyer, K. H. Strobl, and R. Triebel, “The Probabilistic Robot Kinematics Model and its
 * Application to Sensor Fusion,” in 2022 IEEE/RSJ International Conference on Intelligent Robots
 * and Systems (IROS), Oct. 2022, pp. 3263–3270. doi: 10.1109/IROS47612.2022.9981399.
 *
...
SE3WithCov::SE3WithCov(const Pose3f& pose, const Matrix6f& cov) : Pose3f(pose), covariance(cov) {}
...
SE3WithCov SE3WithCov::operator*(const SE3WithCov& other) const {
  ...
  Matrix6f newCov =
      adjointInverseOther * covariance * adjointInverseOther.transpose() + other.covariance;
