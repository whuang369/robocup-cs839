...
Geometry::Circle Geometry::getCircle(const Vector2i& point1, const Vector2i& point2,
                                     const Vector2i& point3) {
  ...
  if (temp == 0)
    ...
    circle.radius =
        0.5f * std::sqrt(((sqr(x1 - x2) + sqr(y1 - y2)) * (sqr(x1 - x3) + sqr(y1 - y3)) *
                          (sqr(x2 - x3) + sqr(y2 - y3))) /
                         sqr(temp));
  if (temp == 0)
    circle.center.x() = 0;
  else
    circle.center.x() = (sqr(x3) * (y1 - y2) + (sqr(x1) + (y1 - y2) * (y1 - y3)) * (y2 - y3) +
                         sqr(x2) * (-y1 + y3)) /
                        (-2.0f * temp);
  if (temp == 0)
    circle.center.y() = 0;
    ...
    circle.center.y() =
        (sqr(x1) * (x2 - x3) + sqr(x2) * x3 + x3 * (-sqr(y1) + sqr(y2)) -
         x2 * (+sqr(x3) - sqr(y1) + sqr(y3)) + x1 * (-sqr(x2) + sqr(x3) - sqr(y2) + sqr(y3))) /
        (2.0f * temp);
  return circle;
}

void Geometry::PixeledLine::calculatePixels(const int x1, const int y1, const int x2, const int y2,
                                            const int stepSize) {
...
bool Geometry::getIntersectionOfLines(const Line& line1, const Line& line2,
...
int Geometry::getIntersectionOfCircles(const Circle& c0, const Circle& c1, Vector2f& p1,
                                       Vector2f& p2) {
...
int Geometry::getIntersectionOfLineAndCircle(const Line& line, const Circle& circle,
...
bool Geometry::getIntersectionOfRaysFactor(const Line& ray1, const Line& ray2, float& factor) {
  const float divisor =
      ray2.direction.x() * ray1.direction.y() - ray1.direction.x() * ray2.direction.y();
  if (divisor == 0) return false;
  const float k = (ray2.direction.y() * ray1.base.x() - ray2.direction.y() * ray2.base.x() -
                   ray2.direction.x() * ray1.base.y() + ray2.direction.x() * ray2.base.y()) /
                  divisor;
  const float l = (ray1.direction.y() * ray1.base.x() - ray1.direction.y() * ray2.base.x() -
                   ray1.direction.x() * ray1.base.y() + ray1.direction.x() * ray2.base.y()) /
                  divisor;
  if ((k >= 0) && (l >= 0) && (k <= 1) && (l <= 1)) {
    factor = k;
    return true;
...
bool Geometry::getIntersectionOfLineAndConvexPolygon(const std::vector<Vector2f>& polygon,
                                                     const Line& direction, Vector2f& intersection,
                                                     const bool isCCW, Line* intersectedLine) {
  ...
  for (size_t i = 0; i < polygon.size(); ++i) {
    ...
    const bool isLeftP1 =
        Geometry::isPointLeftOfLine(direction.base, direction.base + direction.direction, p1);
    const bool isLeftP2 =
    ...
    if (hasIntersection &&
      ...
      if (intersectedLine) *intersectedLine = polygonLine;
      return true;
    }
  }
  return false;
...
float Geometry::getDistanceToLineSigned(const Line& line, const Vector2f& point) {
  if (line.direction.x() == 0 && line.direction.y() == 0) return distance(point, line.base);
...
bool Geometry::isPointInsideRectangle(const Vector2f& bottomLeftCorner,
                                      const Vector2f& topRightCorner, const Vector2f& point) {
  return (bottomLeftCorner.x() <= point.x() && point.x() <= topRightCorner.x() &&
          bottomLeftCorner.y() <= point.y() && point.y() <= topRightCorner.y());
}

bool Geometry::isPointInsideRectangle2(const Vector2f& corner1, const Vector2f& corner2,
...
bool Geometry::isPointInsideRectangle(const Vector2i& bottomLeftCorner,
                                      const Vector2i& topRightCorner, const Vector2i& point) {
  return (bottomLeftCorner.x() <= point.x() && point.x() <= topRightCorner.x() &&
          bottomLeftCorner.y() <= point.y() && point.y() <= topRightCorner.y());
}
...
int Geometry::ccw(const Vector2f& p0, const Vector2f& p1, const Vector2f& p2) {
  ...
  if ((dx1 * dx2 < 0.0f) || (dy1 * dy2 < 0.0f)) return -1;
  if ((dx1 * dx1 + dy1 * dy1) >= (dx2 * dx2 + dy2 * dy2)) return 0;
  return 1;
}

bool Geometry::isPointInsideConvexPolygon(const Vector2f polygon[], const int numberOfPoints,
                                          const Vector2f& point) {
  ...
  for (int i = 1; i < numberOfPoints; i++) {
    ...
    if (currentOrientation == 0) return true;
...
bool Geometry::isPointInsidePolygon(const Vector2f& point, const std::vector<Vector2f>& V) {
  ...
  for (i = 0; i < static_cast<int>(V.size()); ++i) {
    if ((V[i].y() < point.y() && V[j].y() >= point.y()) ||
        (V[j].y() < point.y() && V[i].y() >= point.y())) {
      if (V[i].x() + (point.y() - V[i].y()) / (V[j].y() - V[i].y()) * (V[j].x() - V[i].x()) <
          point.x()) {
        oddNodes = !oddNodes;
      }
    }
...
bool Geometry::checkIntersectionOfLines(const Vector2f& l1p1, const Vector2f& l1p2,
...
bool Geometry::clipPointInsideRectangle(const Vector2i& bottomLeftCorner,
                                        const Vector2i& topRightCorner, Vector2i& point) {
  bool clipped = false;
...
bool Geometry::clipPointInsideRectangle(const Vector2i& bottomLeftCorner,
                                        const Vector2i& topRightCorner, Vector2f& point) {
  bool clipped = false;
  if (point.x() < bottomLeftCorner.x()) {
...
bool Geometry::clipPointToPolygonBorder(const std::vector<Vector2f>& polygon, Vector2f& point) {
  ...
  if (min == 0.f) return false;

  point = getOrthogonalProjectionOfPointOnEdge(
      polygon[n], polygon[n != polygon.size() - 1 ? n + 1 : 0] - polygon[n], point);
  return true;
}
...
bool Geometry::circleIntersectsAxisAlignedRectangle(const Vector2f& cp, float r, const Vector2f& p1,
                                                    const Vector2f& p2) {
  ...
  if (cp.x() < xMin - r || cp.x() > xMax + r || cp.y() < yMin - r || cp.y() > yMax + r) {
    //... it cannot intersect or be inside of the rectangle.
    return false;
  }
  // The previous check should cover most situations. However, it is not
  // valid close to the corners of the rectangle, where the x/y coordinates of the center of the
  // circle could be closer than r to the rectangle edges whilst the circle does not intersect
  // anyway.
  else {
...
bool Geometry::getIntersectionPointsOfLineAndRectangle(const Vector2i& bottomLeft,
                                                       const Vector2i& topRight,
                                                       const Geometry::Line& line, Vector2i& point1,
                                                       Vector2i& point2) {
  int foundPoints = 0;
  Vector2f point[2];
  if (line.direction.x() != 0) {
    const float y1 =
        line.base.y() + (bottomLeft.x() - line.base.x()) * line.direction.y() / line.direction.x();
    if ((y1 >= bottomLeft.y()) && (y1 <= topRight.y())) {
      point[foundPoints].x() = static_cast<float>(bottomLeft.x());
      point[foundPoints++].y() = y1;
    }
    const float y2 =
        line.base.y() + (topRight.x() - line.base.x()) * line.direction.y() / line.direction.x();
  ...
  if (line.direction.y() != 0) {
    const float x1 =
        line.base.x() + (bottomLeft.y() - line.base.y()) * line.direction.x() / line.direction.y();
    ...
    const float x2 =
        line.base.x() + (topRight.y() - line.base.y()) * line.direction.x() / line.direction.y();
...
bool Geometry::getIntersectionPointsOfLineAndRectangle(const Vector2f& bottomLeft,
                                                       const Vector2f& topRight,
                                                       const Geometry::Line& line, Vector2f& point1,
                                                       Vector2f& point2) {
  int foundPoints = 0;
  Vector2f point[2];
  if (line.direction.x() != 0) {
  ...
  if (line.direction.y() != 0) {
    ...
    if ((x2 >= bottomLeft.x()) && (x2 <= topRight.x()) && (foundPoints < 2)) {
      ...
      if ((foundPoints == 0) || ((point[0] - point[1]).norm() > 0.1f)) foundPoints++;
    }
  }
  switch (foundPoints) {
    case 1:
...
bool Geometry::isPointLeftOfLine(const Vector2f& start, const Vector2f& end,
                                 const Vector2f& point) {
  return ((end.x() - start.x()) * (point.y() - start.y()) -
          (end.y() - start.y()) * (point.x() - start.x())) > 0.f;
}

Vector2f Geometry::getOrthogonalProjectionOfPointOnLine(const Vector2f& base, const Vector2f& dir,
                                                        const Vector2f& point) {
...
Vector2f Geometry::getOrthogonalProjectionOfPointOnEdge(const Vector2f& base, const Vector2f& dir,
                                                        const Vector2f& point) {
...
bool Geometry::isPointInsideArc(const Vector2f& point, const Vector2f& center,
                                const Rangea& angleRange, const float radius) {
