...
bool LeastSquares::LineFitter::fit(Vector2f& n0, float& d) const {
  ...
  const Eigen::SelfAdjointEigenSolver<Matrix2f> solver(
      Eigen::SelfAdjointEigenSolver<Matrix2f>().computeDirect(
          (Matrix2f() << var.x(), covXY, covXY, var.y()).finished()));
...
bool LeastSquares::CircleFitter::fit(Vector2f& center, float& radius) const {
  ...
  if (Approx::isZero(divisor)) return false;

  const float coeff1 = 2.f * nInv * sum.squaredNorm();
  const float coeff2 = -2.f * sumXY;
  const Matrix2f m1((Matrix2f() << (coeff1 - 3.f * sum2.x() - sum2.y()), coeff2, coeff2,
                     (coeff1 - 3.f * sum2.y() - sum2.x()))
                        .finished());

  const Matrix2f m2((Matrix2f() << var.y(), -covXY, -covXY, var.x()).finished());
