...
void InExprMemory::inFloat(float& value) {
  ...
  if (*ptr != '\0') ERROR("Value could not be parsed: " << str);
}
...
InExprMapFile::InExprMapFile(const std::string& name,
                             const std::unordered_map<std::string, float>& symbols,
                             unsigned errorMask)
    : InMapFile(name, errorMask), symbols(symbols) {}
