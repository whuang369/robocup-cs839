...
void OutText::writeString(const char* value, PhysicalOutStream& stream) {
  ...
  if (containsSpaces) stream.writeToStream("\"", 1);
  for (; *value; ++value)
...
void OutText::writeData(const void* p, size_t size, PhysicalOutStream& stream) {
  for (size_t i = 0; i < size; ++i) writeChar(*reinterpret_cast<const char*&>(p)++, stream);
}
...
void OutTextRaw::writeAngle(const Angle& d, PhysicalOutStream& stream) {
  if (d == Angle(10000.f) || d == Angle(20000.f) /* SensorData::off || SensorData::ignore */)
...
void OutFile::writeToStream(const void* p, size_t size) {
  if (stream != nullptr) stream->write(p, size);
}

OutMemory::OutMemory(OutMemory&& other)
    : buffer(other.buffer), reserved(other.reserved), bytes(other.bytes), dynamic(other.dynamic) {
...
void OutMemory::writeToStream(const void* p, size_t size) {
  if (bytes + size > reserved) {
    if (dynamic) {
      reserved =
          std::max(std::min(reserved, std::numeric_limits<size_t>::max() / 2) * 2, bytes + size);
...
void OutMap::outUInt(unsigned int value) {
  if (stack.back().type != -1) *stream << value;
}

void OutMap::outString(const char* value) {
  char buf[2] = {0};
  bool containsSpecialChars =
      !*value || *value == '"' || strcspn(value, " \n\r\t=,;]}") < strlen(value);
  if (containsSpecialChars) *stream << "\"";
  for (; *value; ++value)
...
void OutMap::select(const char* name, int type, const char* enumType) {
  ...
  if (!stack.empty()) {
    ...
    if (!e.hasSubEntries) {
      ...
      if (mode != singleLine) indentation += "  ";
...
void OutMap::deselect() {
  Entry& e = stack.back();
  if (e.hasSubEntries) {
    if (mode != singleLine) indentation = indentation.substr(2);
    if (e.type == -1)  // array
    {
      writeLn();
      *stream << indentation << "]";
      ...
      *stream << indentation << "}";
    if (mode == singleLineInnermost) flush(true);
...
OutMapFile::OutMapFile(const std::string& name, bool singleLineInnermost, size_t maxCollapsedLength)
    : OutMap(stream, singleLineInnermost ? OutMap::singleLineInnermost : multiLine,
             maxCollapsedLength),
      stream(name) {}
