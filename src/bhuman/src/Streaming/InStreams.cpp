...
void InText::readString(std::string& value, PhysicalInStream& stream) {
  ...
  if (containsSpaces && !isEof(stream)) nextChar(stream);
  while (!isEof(stream) && (containsSpaces || !isWhitespace()) &&
         (!containsSpaces || theChar != '"')) {
    if (theChar == '\\') {
      nextChar(stream);
      if (theChar == 'n')
        theChar = '\n';
    ...
    value += theChar;
    if (!isEof(stream)) nextChar(stream);
  }
  if (containsSpaces && !isEof(stream)) nextChar(stream);
  skipWhitespace(stream);
}
...
void InText::readData(void* p, size_t size, PhysicalInStream& stream) {
  for (size_t i = 0; i < size; ++i) readChar(*reinterpret_cast<char*&>(p)++, stream);
}
...
void InText::skipWhitespace(PhysicalInStream& stream) {
  while (!isEof(stream) && isWhitespace()) nextChar(stream);
}
...
bool InText::expectString(const std::string& str, PhysicalInStream& stream) {
  ...
  if (str.length()) {
    while (*p) {
      if (isEof(stream) || theChar != *p) return false;
      ++p;
      nextChar(stream);
    }
  }
    ...
    while (!isEof(stream)) {
      ...
      if (fileEntry == section) {
        if (theChar == '[')  // handle empty section
          while (!isEof(stream)) InText::nextChar(stream);
        break;
      }
    }
    readSection = true;
...
bool InConfig::isWhitespace() {
  return (theChar == '/' && (theNextChar == '*' || theNextChar == '/')) || theChar == '#' ||
         InText::isWhitespace();
}

void InConfig::skipWhitespace(PhysicalInStream& stream) {
  while (!isEof(stream) && isWhitespace()) {
    while (!isEof(stream) && InText::isWhitespace()) nextChar(stream);
    if (!isEof(stream)) {
...
void InConfig::skipComment(PhysicalInStream& stream) {
  ...
  nextChar(stream);
  nextChar(stream);
  while (!isEof(stream) && (theChar != '*' || theNextChar != '/')) nextChar(stream);
...
void InFile::open(const std::string& name) {
  if (stream == nullptr) stream = new File(name, "rb");
}

void InFile::readFromStream(void* p, size_t size) {
  if (stream != nullptr) stream->read(p, size);
}

void InFile::skipInStream(size_t size) {
  if (stream != nullptr) stream->skip(size);
}
...
void InMap::printError([[maybe_unused]] const std::string& msg, ErrorType errorType) {
...
#ifdef TARGET_ROBOT
    FAIL(name << (name.empty() || path.empty() ? "" : ", ") << path
              << (name.empty() && path.empty() ? "" : ": ") << msg);
#else
    OUTPUT_ERROR(name << (name.empty() || path.empty() ? "" : ", ") << path
                      << (name.empty() && path.empty() ? "" : ": ") << msg);
#endif
  }
}
...
void InMap::select(const char* name, int type, const char* enumType) {
  ...
  const SimpleMap::Value* value =
      stack.empty() ? (const SimpleMap::Value*)*map : stack.back().value;
  if (!value)                                           // invalid
  ...
  if (errorMask & bit(unusedAttribute)) {
    ...
    if (record)
      for (const auto& [name, attribute] : *record)
        if (!attribute->accessed) printError("unused attribute " + name, unusedAttribute);
  }

  stack.pop_back();
}

InMapFile::InMapFile(const std::string& name, unsigned errorMask) : InMap(errorMask), stream(name) {
  if (stream.exists()) parse(stream, stream.getFile()->getFullName());
}

InMapMemory::InMapMemory(const void* memory, size_t size, unsigned errorMask)
    : InMap(errorMask), stream(memory, size) {
  parse(stream);
}
