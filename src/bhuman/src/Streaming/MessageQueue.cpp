...
void MessageQueue::OutQueue::writeToStream(const void* p, size_t size) {
  if (queue) {
    if (queue->ensureCapacity(queue->used + size, maxCapacity)) {
      ...
      queue->used += size;
      reinterpret_cast<MessageHeader*>(queue->buffer + originalSize)->size +=
...
size_t MessageQueue::calcMaxCapacity(MessageID id) const {
  constexpr unsigned unprotected =
      bit(idDebugDrawing - numOfDataMessageIDs) | bit(idDebugDrawing3D - numOfDataMessageIDs) |
      bit(idDebugImage - numOfDataMessageIDs) | bit(idPlot - numOfDataMessageIDs) |
      bit(idText - numOfDataMessageIDs);

  return id <= idFrameFinished ||
                 (id >= numOfDataMessageIDs && !(bit(id - numOfDataMessageIDs) & unprotected))
             ? maxCapacity
             : maxCapacity - protectedCapacity;
}
...
MessageQueue& MessageQueue::operator<<(const std::pair<const_iterator, const_iterator>& range) {
  ...
  if (ensureCapacity(used + srcSize, maxCapacity - protectedCapacity)) {
    ...
    copyMessages(srcSize, [&](void* dest, size_t size) {
      if (dest) std::memcpy(dest, src, size);
      src += size;
    });
  }
  return *this;
