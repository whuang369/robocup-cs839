...
File::File(const std::string& name, const char* mode, bool tryAlternatives) {
  ...
  if (tryAlternatives) {
    ...
    if (stream) fullName = names.back();
  }
}

File::~File() {
  if (stream != 0) fclose(static_cast<FILE*>(stream));
}

std::list<std::string> File::getFullNames(const std::string& name) {
  std::list<std::string> names;
  if ((name[0] != '.' || (name.size() >= 2 && name[1] == '.')) &&
      !isAbsolute(name))  // given path is relative to getBHDir()
  {
    for (const std::string& dir : searchPath) names.push_back(dir + name);
...
const char* File::getBHDir() {
  ...
  if (!dir[0]) {
    ...
    if (len >= sizeof(dir) - 8) len = sizeof(dir) - 8;
