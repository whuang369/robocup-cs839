...
void UKFPose2D::motionUpdate(const Pose2f& odometryOffset, const Pose2f& filterProcessDeviation,
                             const Pose2f& odometryDeviation,
                             const Vector2f& odometryRotationDeviation) {
  generateSigmaPoints();
  ...
  mean = sigmaPoints[0];
  for (int i = 1; i < 7; ++i) mean += sigmaPoints[i];
  mean *= 1.f / 7.f;
...
void UKFPose2D::landmarkSensorUpdate(const Vector2f& landmarkPosition, const Vector2f& reading,
  ...
  for (int i = 1; i < 7; ++i) landmarkReadingMean += landmarkReadings[i];
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    landmarkReadingAndMeanCov +=
        (Matrix2x3f() << d1 * l(0, i), d1 * l(1, i), d1 * l(2, i)).finished();
  ...
  const Matrix3x2f kalmanGain =
      landmarkReadingAndMeanCov.transpose() * (landmarkReadingCov + readingCov).inverse();
...
void UKFPose2D::lineSensorUpdate(bool lineIsParallelToWorldModelXAxis, const Vector2f& reading,
                                 const Matrix2f& readingCov) {
  generateSigmaPoints();
  ...
  if (lineIsParallelToWorldModelXAxis)
    for (int i = 0; i < 7; ++i) lineReadings[i] = Vector2f(sigmaPoints[i].y(), sigmaPoints[i].z());
  else
    for (int i = 0; i < 7; ++i) lineReadings[i] = Vector2f(sigmaPoints[i].x(), sigmaPoints[i].z());

  // computeMeanOfLineReadings
  Vector2f lineReadingMean = lineReadings[0];
  for (int i = 1; i < 7; ++i) lineReadingMean += lineReadings[i];
  ...
  const Matrix3x2f kalmanGain =
      lineReadingAndMeanCov.transpose() * (lineReadingCov + readingCov).inverse();
...
void UKFPose2D::poseSensorUpdate(const Vector3f& reading, const Matrix3f& readingCov) {
  ...
  for (int i = 0; i < 7; ++i) poseReadings[i] = sigmaPoints[i];

  // computeMeanOfPoseReadings
  Vector3f poseReadingMean = poseReadings[0];
  for (int i = 1; i < 7; ++i) poseReadingMean += poseReadings[i];
  ...
  const Matrix3f kalmanGain =
      poseReadingAndMeanCov.transpose() * (poseReadingCov + readingCov).inverse();
