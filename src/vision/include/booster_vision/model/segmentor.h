...
class YoloV8Segmentor {
 public:
  ...
  void setConfidenceThreshold(float confidence_threshold) { confidence_ = confidence_threshold; }

  float getConfidenceThreshold() { return confidence_; }
  ...
  static cv::Mat DrawSegmentation(const cv::Mat &img,
                                  const std::vector<SegmentationRes> &detections);
