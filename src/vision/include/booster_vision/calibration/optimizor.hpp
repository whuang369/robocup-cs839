...
void Project(const T *const point, const T *const intr, const T *const d, T *uv) {
  ...
  T x_prime = x * (T(1) + d[0] * r2 + d[1] * r4 + d[4] * r6) + T(2) * d[2] * x * y +
              d[3] * (r2 + T(2) * x * x);
  T y_prime = y * (T(1) + d[0] * r2 + d[1] * r4 + d[4] * r6) + d[2] * (r2 + T(2) * y * y) +
              T(2) * d[3] * x * y;
 ...
 public:
  ExtrinsicsOptimizer(const cv::Point3f &object_point, const cv::Point2f &uv,
      ...
      : object_point_(object_point),
        uv_(uv),
        q_base2head_(q_base2head),
        t_base2head_(t_base2head),
        intr_(intr) {}
  ...
  bool operator()(const T *const q_head2cam_data, const T *const t_head2cam_data,
    ...
    Eigen::Matrix<T, 3, 1> t_base2cam =
        q_head2cam * Eigen::Matrix<T, 3, 1>(t_base2head_.cast<T>()) +
        Eigen::Matrix<T, 3, 1>(t_head2cam_data);
    ...
    Eigen::Matrix<T, 3, 1> t_board2cam =
        q_base2cam * Eigen::Matrix<T, 3, 1>(t_board2base_data) + t_base2cam;
