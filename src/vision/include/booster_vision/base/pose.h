...
struct Pose {
 public:
  Pose() = default;
  Pose(const float &x, const float &y, const float &z, const float &roll, const float &pitch,
       const float &yaw);
  Pose(cv::Mat &pose) : mat_pose(pose){};
  ...
  cv::Mat toCVMat() const { return mat_pose; }
  geometry_msgs::msg::TransformStamped toRosTFMsg();

  // geters
  cv::Mat getRotationMatrix() const { return mat_pose(cv::Rect(0, 0, 3, 3)).clone(); }

  cv::Mat getTranslationVecMatrix() const { return mat_pose(cv::Rect(3, 0, 1, 3)).clone(); }
