...
struct Intrinsics {
  ...
  Intrinsics(const cv::Mat intr, const std::vector<float> &distortion_coeffs,
             const DistortionModel &model);
  Intrinsics(const cv::Mat intr) : Intrinsics(intr, std::vector<float>(), DistortionModel::kNone) {}
  Intrinsics(float fx, float fy, float cx, float cy, const std::vector<float> &distortion_coeffs,
...
struct convert<booster_vision::Intrinsics> {
  ...
  static bool decode(const Node &node, booster_vision::Intrinsics &intr) {
    ...
    intr.model = static_cast<booster_vision::Intrinsics::DistortionModel>(
        node["distortion_model"].as<int>());
