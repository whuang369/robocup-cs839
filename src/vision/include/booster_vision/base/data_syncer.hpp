...
struct DataBlock {
  ...
  DataBlock() : timestamp(0) {}
  DataBlock(const T &data, double timestamp) : data(data), timestamp(timestamp) {}
};
...
struct SyncedDataBlock {
  ...
  SyncedDataBlock(const SyncedDataBlock &other) : pose_data(other.pose_data) {
