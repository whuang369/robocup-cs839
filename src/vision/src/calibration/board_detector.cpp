...
Pose BoardDetector::getBoardPose() {
  cv::Mat rvec;
  cv::Mat tvec;
  bool res = cv::solvePnP(board_points_, board_uvs_, intr_.get_intrinsics_matrix(),
                          intr_.distortion_coeffs, rvec, tvec);
  if (!res) {
    return Pose();
  }
...
std::vector<cv::Point2f> BoardDetector::getBoardUVsSubpixel() {
  ...
  cv::cornerSubPix(gray_, board_uvs_, cv::Size(11, 11), cv::Size(-1, -1),
