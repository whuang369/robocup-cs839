...
double Compute2dError(const std::vector<std::vector<cv::Point3f>> &all_corners_3d,
                      const std::vector<std::vector<cv::Point2f>> &all_corners_2d,
                      const std::vector<Pose> p_head2bases, const Pose p_board2base,
                      const Pose p_eye2head, const Intrinsics &intr) {
  double avg_error = 0;
...
void EyeInHandCalibration(double *reprojection_error, Pose *p_cam2head_best,
                          Pose *p_board2base_best, const std::vector<Pose> &p_board2cameras,
                          const std::vector<Pose> &p_head2bases,
                          const std::vector<std::vector<cv::Point3f>> &all_corners_3d,
                          const std::vector<std::vector<cv::Point2f>> &all_corners_2d,
                          const Intrinsics &intr, bool optimization) {
  // prepare data
  ...
  *reprojection_error = std::numeric_limits<double>::max();
  for (int method = 0; method < 5; method++) {
    ...
    cv::Mat t_eye2head;
    try {
      cv::calibrateHandEye(r_head2bases, t_head2bases, r_board2cameras, t_board2cameras, r_eye2head,
                           t_eye2head, static_cast<cv::HandEyeCalibrationMethod>(method));
    ...
    double error = Compute2dError(all_corners_3d, all_corners_2d, p_head2bases, p_board2base,
                                  p_eye2head, intr);
    ...
    if (!optimization) {
      ...
      Eigen::Quaterniond q_head2cam =
          Eigen::Map<Eigen::Quaterniond>(p_head2cam.getQuaternionVec().data());
      Eigen::Vector3d t_head2cam =
          Eigen::Map<Eigen::Vector3f>(p_head2cam.getTranslationVec().data()).cast<double>();

      // board2base
      Eigen::Quaterniond q_board2base =
          Eigen::Map<Eigen::Quaterniond>(p_board2base.getQuaternionVec().data());
      Eigen::Vector3d t_board2base =
          Eigen::Map<Eigen::Vector3f>(p_board2base.getTranslationVec().data()).cast<double>();
      ...
      for (size_t i = 0; i < all_corners_2d.size(); i++) {
        for (size_t j = 0; j < all_corners_2d[i].size(); j++) {
          ...
          Eigen::Quaterniond q_base2head =
              Eigen::Map<Eigen::Quaterniond>(p_base2head.getQuaternionVec().data());
          Eigen::Vector3d t_base2head =
              Eigen::Map<Eigen::Vector3f>(p_base2head.getTranslationVec().data()).cast<double>();

          ceres::CostFunction *cost_function =
              new ceres::AutoDiffCostFunction<ExtrinsicsOptimizer, 2, 4, 3, 4, 3>(
                  new ExtrinsicsOptimizer(corner_3d, corner_2d, q_base2head, t_base2head, intr));
          problem.AddResidualBlock(cost_function, nullptr, q_head2cam.coeffs().data(),
                                   t_head2cam.data(), q_board2base.coeffs().data(),
                                   t_board2base.data());
        }
      }
      ...
      Pose p_board2base_opt(t_board2base_opt(0), t_board2base_opt(1), t_board2base_opt(2),
                            q_board2base_opt.x(), q_board2base_opt.y(), q_board2base_opt.z(),
                            q_board2base_opt.w());
      Pose p_head2cam_opt(t_head2cam_opt(0), t_head2cam_opt(1), t_head2cam_opt(2),
                          q_head2cam_opt.x(), q_head2cam_opt.y(), q_head2cam_opt.z(),
                          q_head2cam_opt.w());
