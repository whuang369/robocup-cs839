...
class CalibrationNode : public rclcpp::Node {
 public:
  ...
  void Init(const std::string cfg_path, bool offline_mode = false,
            std::string calibration_mode = "handeye");
...
CalibrationNode::CalibrationNode(const std::string &node_name) : rclcpp::Node(node_name) {
...
void CalibrationNode::Init(const std::string cfg_path, bool is_offline,
  ...
  board_detector_ =
      std::make_shared<BoardDetector>(cv::Size(board_w_, board_h_), board_square_size_, intr_);
...
void CalibrationNode::RunExtrinsicCalibrationProcess(const SyncedDataBlock &data_block) {
  if (!is_offline_ && new_log_path_) {
    log_path_ =
        std::string(std::getenv("HOME")) + "/Workspace/calibration_log/handeye/" + getTimeString();
    new_log_path_ = false;
  }
  ...
  cv::putText(display_img, progress_status_text, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 0.7,
              cv::Scalar(0, 0, 255), 2);
  cv::putText(display_img, "press h for help info in terminal", cv::Point(10, 50),
  ...
  switch (key) {
    case 's': {
      ...
      if (found) {
        ...
        cv::putText(mask, std::to_string(cali_data_.size()),
                    cv::Point(corners[0].x, corners[0].y + 10), cv::FONT_HERSHEY_SIMPLEX, 0.5,
                    cv::Scalar(0, 0, 255), 2);
    ...
    case 'c': {
      ...
      EyeInHandCalibration(&reprojection_error, &p_eye2head_best, &p_board2base_best,
                           p_board2cameras, p_head2bases, all_corners_3d_, all_corners_2d_, intr_);
