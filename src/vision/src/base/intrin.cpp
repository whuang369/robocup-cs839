...
Intrinsics::Intrinsics(const cv::Mat intr, const std::vector<float> &distortion_coeffs_value,
                       const DistortionModel &model_value) {
...
Intrinsics::Intrinsics(float fx_value, float fy_value, float cx_value, float cy_value,
                       const std::vector<float> &distortion_coeffs_value,
                       DistortionModel model_value) {
  fx = fx_value;
  fy = fy_value;
  cx = cx_value;
  cy = cy_value;
...
cv::Point3f Intrinsics::BackProject(const cv::Point2f &point, float depth) const {
  ...
  switch (model) {
    ...
    case DistortionModel::kBrownConrady: {
      ...
      cv::undistortPoints(distorted_points, undistort_points, get_intrinsics_matrix(),
                          distortion_coeffs);
    ...
    case DistortionModel::kInverseBrownConrady: {
      ...
      float r2 = x * x + y * y;
      float f = 1 + distortion_coeffs[0] * r2 + distortion_coeffs[1] * r2 * r2 +
