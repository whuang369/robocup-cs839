...
SyncedDataBlock DataSyncer::getSyncedDataBlock() {
  std::string color_file_path =
      data_dir_ + "/color_" + std::to_string(time_stamp_list_[data_index_]) + ".jpg";
  std::string depth_file_path =
      data_dir_ + "/depth_" + std::to_string(time_stamp_list_[data_index_]) + ".png";
  std::string pose_file_path =
      data_dir_ + "/pose_" + std::to_string(time_stamp_list_[data_index_]) + ".yaml";
...
SyncedDataBlock DataSyncer::getSyncedDataBlock(const ColorDataBlock &color_data) {
  ...
  if (enable_depth_) {
    ...
    for (DepthBuffer::reverse_iterator it = depth_buffer_cp.rbegin();
         it != depth_buffer_cp.rbegin() + kDepthBufferLength; ++it) {
  ...
  for (PoseBuffer::reverse_iterator it = pose_buffer_cp.rbegin();
       it != pose_buffer_cp.rbegin() + kPoseBufferLength; ++it) {
    double diff = std::abs(it->timestamp - color_timestamp);
