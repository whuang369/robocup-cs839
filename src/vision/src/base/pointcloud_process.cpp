...
void CreatePointCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud, const cv::Mat &depth_image,
                      const cv::Mat &rgb_image, const cv::Rect &bbox,
...
void CreatePointCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud, const cv::Mat &depth_image,
                      const cv::Mat &rgb_image, const booster_vision::Intrinsics &intrinsics) {
  cv::Rect bbox(0, 0, depth_image.cols, depth_image.rows);
  CreatePointCloud(cloud, depth_image, rgb_image, bbox, intrinsics);
}

void DownSamplePointCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &processed_cloud,
...
void PointCloudNoiseRemoval(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &processed_cloud,
                            const int neighbour_count, const float multiplier,
                            const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud) {
...
void ClusterPointCloud(std::vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr> &clustered_clouds,
                       const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud,
  ...
  for (const auto &indices : cluster_indices) {
    ...
    //   std::cout << "cluster size: " << indices.indices.size() << " smaller than threshold, skip!"
    //   << std::endl; continue; // skip cluster with size smaller than fifth of the input cloud
    // }
...
void SphereFitting(std::vector<float> &sphere, float &confidence,
  ...
  if (std::abs(coefficients.values[3] - radius_threshold) > 0.02) {
    std::cout << "raidus " << coefficients.values[3] << " higher than expected " << radius_threshold
              << std::endl;
