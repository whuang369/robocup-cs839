...
Pose::Pose(const float &x, const float &y, const float &z, const float &roll, const float &pitch,
           const float &yaw) {
  cv::Mat Rx =
      (cv::Mat_<double>(3, 3) << 1, 0, 0, 0, cos(roll), -sin(roll), 0, sin(roll), cos(roll));

  cv::Mat Ry =
      (cv::Mat_<double>(3, 3) << cos(pitch), 0, sin(pitch), 0, 1, 0, -sin(pitch), 0, cos(pitch));
...
Pose::Pose(const geometry_msgs::msg::TransformStamped &msg) {
  ...
  cv::Mat R =
      (cv::Mat_<float>(3, 3) << rotation_matrix[0][0], rotation_matrix[0][1], rotation_matrix[0][2],
       rotation_matrix[1][0], rotation_matrix[1][1], rotation_matrix[1][2], rotation_matrix[2][0],
       rotation_matrix[2][1], rotation_matrix[2][2]);

  R.copyTo(mat_pose(cv::Rect(0, 0, 3, 3)));
}

Pose::Pose(float x, float y, float z, float qx, float qy, float qz, float qw) {
  mat_pose.at<float>(0, 3) = x;
  mat_pose.at<float>(1, 3) = y;
  mat_pose.at<float>(2, 3) = z;
...
geometry_msgs::msg::TransformStamped Pose::toRosTFMsg() {
  ...
  tf2::Matrix3x3 tf_rotation(
      mat_pose.at<float>(0, 0), mat_pose.at<float>(0, 1), mat_pose.at<float>(0, 2),
