...
const std::vector<std::string> YoloV8Detector::kClassLabels{
    "Ball",   "Goalpost",     "Person",   "LCross",   "TCross",
    "XCross", "PenaltyPoint", "Opponent", "BRMarker",
};
...
cv::Mat YoloV8Detector::DrawDetection(const cv::Mat &img,
  ...
  for (const auto &detection : detections) {
    ...
    cv::putText(img_out, kClassLabels[detection.class_id],
                cv::Point(detection.bbox.x, detection.bbox.y + 5), cv::FONT_HERSHEY_SIMPLEX, 0.5,
