...
class Stablizer {
 public:
  Stablizer() { _timespan = chr::milliseconds(2000); };

  void setTimespan(int msec) { _timespan = chr::milliseconds(msec); };
 ...
 private:
  ...
  void _removeOldData() {
    ...
    int i;
    for (i = 0; i < _t.size(); i++) {
      if (_t[i] + _timespan > curTime) break;
    }
