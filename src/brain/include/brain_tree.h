...
class CamFindBall : public SyncActionNode {
 ...
 private:
  double _cmdSequence[6][2];  // The sequence of actions for finding the ball, in which the robot
                              // looks towards these positions in order.
  rclcpp::Time _timeLastCmd;  // The time of the last command execution, used to ensure there is a
                              // time interval between commands.
  int _cmdIndex;              // The current step in the cmdSequence that is being executed.
  long _cmdIntervalMSec;  // The time interval (in milliseconds) between executing actions in the
                          // sequence.
  long _cmdRestartIntervalMSec;  // If the time since the last execution exceeds this value, the
                                 // sequence will restart from step 0.

  Brain *brain;
};

// The robot performs the action of finding the ball, which needs to be used in conjunction with
// CamFindBall.
 ...
 public:
  ...
  static PortsList providedPorts() {
    return {
        ...
        InputPort<double>(
            "vtheta_limit", 0.1,
            "Maximum angular velocity for real-time direction adjustment while chasing the ball"),
  ...
  static PortsList providedPorts() {
    return {
        InputPort<double>("turn_threshold", 0.2,
                          "If the angle to the ball exceeds this value, the robot will first turn "
                          "to face the ball"),
        ...
        InputPort<double>("vtheta_limit", 0.4,
                          "Limit for vtheta during adjustment, [-limit, limit]"),
        InputPort<double>("max_range", 1.5,
        ...
        InputPort<string>("position", "offense",
                          "offense | defense, determines which direction to kick the ball"),
    };
  }
...
class Kick : public StatefulActionNode {
 public:
  ...
  static PortsList providedPorts() {
    return {
        InputPort<int>("min_msec_kick", 500,
                       "The minimum duration (in milliseconds) for executing a kick action"),
        InputPort<int>("msec_stand", 500,
                       "The number of milliseconds after issuing a stop command"),
 ...
 public:
  ...
  static BT::PortsList providedPorts() {
    return {
        ...
        InputPort<int>("msec_cycle", 6000,
                       "How many milliseconds it takes to complete one full rotation"),
    };
  }
...
// SelfLocate: Uses particle filtering to correct the current position, compensating for odometry
// drift.
class SelfLocate : public SyncActionNode {
 public:
  ...
  static PortsList providedPorts() {
    return {
        // enter_field: Used when entering the field, at this point, the robot is definitely in its
        // own half, and the direction can be further narrowed based on the position of its own
        // goal.
        // trust_direction: Used in normal conditions, where the odom information is generally
        // accurate (the robot has not fallen over).
        // trust_position: Used after the robot has fallen. At this point, x and y are reliable, but
        // the direction is not. (Note: if near the midfield line, the position should also be
        // considered unreliable due to the symmetry of the field).
        // trust_nothing: An extreme case where neither x nor y is reliable, and the robot needs to
        // identify its direction using landmarks.
        // face_forward: The direction facing the opponent's goal, primarily used for testing.
        InputPort<string>("mode", "enter_field",
                          "must be one of [enter_field, trust_direction, trust_position, "
                          "trust_nothing, face_forward]"),
    };
  };
 ...
 public:
  ...
  static BT::PortsList providedPorts() {
    return {
        ...
        InputPort<double>("theta", 0,
                          "Final orientation of the target in the Field coordinate system"),
...
class BlockGoal : public SyncActionNode {
 public:
  BlockGoal(const std::string &name, const NodeConfig &config, Brain *_brain)
      : SyncActionNode(name, config), brain(_brain) {}

  static BT::PortsList providedPorts() {
    return {
        InputPort<double>("dist_to_ball", 2.0,
                          "The distance to the ball when blocking the goal, used to determine "
                          "the position behind the ball"),
        InputPort<double>("long_range_threshold", 1.5,
                          "When the distance to the target point exceeds this value, prioritize "
                          "moving towards it rather than fine-tuning position and orientation"),
...
class RotateForRelocate : public StatefulActionNode {
 public:
  RotateForRelocate(const string &name, const NodeConfig &config, Brain *_brain)
      : StatefulActionNode(name, config), brain(_brain) {}
 ...
 *
 * @param x, y, theta double, the robot's velocity in the x and y directions (m/s) and angular
 * velocity (rad/s) for counterclockwise rotation. Default values are 0. If all values are 0, it is
 * equivalent to issuing a command to make the robot stand still.
 */
...
// The difference from Chase is that Simple Chase just moves towards the ball without circling
// around to the ball's back, and therefore does not require field, localization, or video to run.
class SimpleChase : public SyncActionNode {
 public:
  ...
  static PortsList providedPorts() {
    return {
        InputPort<double>("stop_dist", 1.0,
                          "The distance at which the robot will stop moving towards the ball"),
        InputPort<double>("stop_angle", 0.1,
                          "The angle at which the robot will stop turning towards the ball"),
        InputPort<double>(
            "vy_limit", 0.2,
            "Limit the velocity in the Y direction to prevent instability while walking"),
        InputPort<double>(
            "vx_limit", 0.6,
            "Limit the velocity in the X direction to prevent instability while walking"),
    };
  }
