...
{
    Vector2f percept = Vector2f::Zero();       /**< The position of the perceived landmark (relative to the robot) */
    Matrix2f covPercept = Matrix2f::Identity();  /**< The covariance of the landmark measurement */
    Vector2f model = Vector2f::Zero();         /**< The position of original landmark (in global coordinates) */
};
...
protected:
    ...
    double solveAssignment(const MatrixXd& costMatrix, std::vector<int>& assignment, const double threshold);

    void registerLandmarks(const Pose2f& samplePose,
