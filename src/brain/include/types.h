...
struct FieldDimensions {
  ...
  double goalAreaWidth;      // The width of the goal area.
                         // Note: The penalty area is larger than the goal area; the actual lengths
                         // and widths of the penalty area and the goal area are smaller. This
                         // naming is to be consistent with the competition rules.
};
const FieldDimensions FD_KIDSIZE{9, 6, 1.5, 2.6, 0.75, 2, 5, 1, 3};
const FieldDimensions FD_ADULTSIZE{14, 9, 2.1, 2.6, 1.5, 3, 6, 1, 4};
const FieldDimensions FD_GDC{8.03, 5.95, 1.29, 1.9, 0.75, 0.595, 2.19, 1.64, 3.985};
...
struct Pose2D {
  ...
  double y = 0;
  double theta =
      0;  // rad, counterclockwise is positive starting from the positive direction of the x-axis.
};
...
/// GameObject, used to store the information of important entities in the game, such as Ball,
/// Goalpost, etc. Compared with the detection::DetectedObject in the /detect message, it has more
...
struct GameObject {
  ...
  string info;  // Used to store additional information. For example, for a goalpost object, it can
                // store which goalpost it is.
  Point posToField;  // The position of the object in the field coordinate system. The position is
                     // 2D, ignoring the z value. x is forward and y is leftward.
  double range;      // The straight-line distance from the object to the projection point of the
                     // robot's center on the field plane.
  double pitchToRobot, yawToRobot;  // The pitch and yaw of the object relative to the front of the
                                    // robot, in rad. Downward and leftward are positive.
...
enum class RobotRecoveryState { IS_READY = 0, IS_FALLING = 1, HAS_FALLEN = 2, IS_GETTING_UP = 3 };