...
void Locator::init(FieldDimensions fd, int minMarkerCntParam, double residualToleranceParam,
...
void Locator::calcFieldMarkers(FieldDimensions fd) {
  ...
  fieldMarkers.push_back(
      FieldMarker{'L', (fd.length / 2 - fd.penaltyAreaLength), fd.penaltyAreaWidth / 2});
  fieldMarkers.push_back(
      FieldMarker{'L', (fd.length / 2 - fd.penaltyAreaLength), -fd.penaltyAreaWidth / 2});
  fieldMarkers.push_back(
      FieldMarker{'L', -(fd.length / 2 - fd.penaltyAreaLength), fd.penaltyAreaWidth / 2});
  ...
  // Goal area
  fieldMarkers.push_back(
      FieldMarker{'L', (fd.length / 2 - fd.goalAreaLength), fd.goalAreaWidth / 2});
  fieldMarkers.push_back(
      FieldMarker{'L', (fd.length / 2 - fd.goalAreaLength), -fd.goalAreaWidth / 2});
  fieldMarkers.push_back(
      FieldMarker{'L', -(fd.length / 2 - fd.goalAreaLength), fd.goalAreaWidth / 2});
...
int Locator::genInitialParticles(int num) {
  unsigned long long seed =
      chr::duration_cast<std::chrono::milliseconds>(chr::system_clock::now().time_since_epoch())
          .count();
  srand(seed);
  hypos.resize(num, 6);
...
Pose2D Locator::finalAdjust(vector<FieldMarker> markers_r, Pose2D pose) {
  if (markers_r.size() == 0) return Pose2D{0, 0, 0};
...
bool Locator::isConverged() {
  return ((hypos.col(0).maxCoeff() - hypos.col(0).minCoeff() < convergeTolerance) &&
          (hypos.col(1).maxCoeff() - hypos.col(1).minCoeff() < convergeTolerance) &&
          (hypos.col(2).maxCoeff() - hypos.col(2).minCoeff() < convergeTolerance));
}

int Locator::locateRobot(vector<FieldMarker> markers_r, PoseBox2D constraintsParam, Pose2D &pose,
                         double &avgResidual, int numParticles, double offsetXParam,
                         double offsetYParam, double offsetThetaParam) {
  if (markers_r.size() < minMarkerCnt) return 4;
  ...
  if (calcProbs(markers_r)) return 5;

  for (int i = 0; i < maxIteration; i++) {
    if (isConverged()) {
      avgResidual = bestResidual / markers_r.size();
      if (avgResidual > residualTolerance) return 2;
...
LocateResult Locator::locateRobot(vector<FieldMarker> markers_r, PoseBox2D constraintsParam,
                                  int numParticles, double offsetXParam, double offsetYParam,
