...
void BrainCommunication::initGameControllerBroadcast() {
  try {
    ...
    if (_gc_send_socket < 0) {
      cout << RED_CODE << format("gc socket failed: %s", strerror(errno)) << RESET_CODE << endl;
      throw std::runtime_error(strerror(errno));
    }
    // broadcast
    int gc_broadcast_enable = 1;
    if (setsockopt(_gc_send_socket, SOL_SOCKET, SO_BROADCAST, &gc_broadcast_enable,
                   sizeof(gc_broadcast_enable)) < 0) {
      cout << RED_CODE << format("Failed to set gc SO_BROADCAST: %s", strerror(errno)) << RESET_CODE
           << endl;
      throw std::runtime_error(strerror(errno));
    }
    // target address
...
void BrainCommunication::clearupGameControllerBroadcast() {
  ...
  if (_gc_send_socket >= 0) {
    ...
    _gc_send_socket = -1;
    cout << RED_CODE << format("GameControl send socket has been closed.") << RESET_CODE << endl;
  }
...
void BrainCommunication::initDiscoveryBroadcast() {
  try {
    ...
    if (_discovery_send_socket < 0) {
      cout << RED_CODE << format("socket failed: %s", strerror(errno)) << RESET_CODE << endl;
      throw std::runtime_error(strerror(errno));
    }

    // 设置广播选项
    int broadcast = 1;
    if (setsockopt(_discovery_send_socket, SOL_SOCKET, SO_BROADCAST, &broadcast,
                   sizeof(broadcast)) < 0) {
      cout << RED_CODE << format("Failed to set SO_BROADCAST: %s", strerror(errno)) << RESET_CODE
           << endl;
      throw std::runtime_error(strerror(errno));
    }
...
void BrainCommunication::clearupDiscoveryBroadcast() {
  ...
  if (_discovery_send_socket >= 0) {
    ...
    cout << RED_CODE << format("Discovery send socket has been closed.") << RESET_CODE << endl;
  }
...
void BrainCommunication::initDiscoveryReceiver() {
  try {
    ...
    if (setsockopt(_discovery_recv_socket, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)) < 0) {
      cout << RED_CODE << format("Failed to set SO_REUSEADDR: %s", strerror(errno)) << RESET_CODE
           << endl;
      throw std::runtime_error(strerror(errno));
    }
...
void BrainCommunication::clearupDiscoveryReceiver() {
  ...
  if (_discovery_recv_socket >= 0) {
    ...
    cout << RED_CODE << format("Communication receive socket has been closed.") << RESET_CODE
         << endl;
  }
...
void BrainCommunication::broadcastToGameController() {
  while (_broadcast_gamecontrol_flag) {
    // cout << RED_CODE << format("broadcastToGameController header=%s version=%d teamId=%d,
    // playerId=%d", gc_return_data.header, gc_return_data.version, brain->config->teamId,
...
void BrainCommunication::spinDiscoveryReceiver() {
  ...
  while (_receive_discovery_flag) {
    ssize_t len =
        recvfrom(_discovery_recv_socket, &msg, sizeof(msg), 0, (sockaddr *)&addr, &addr_len);

    if (len < 0) {
      cout << RED_CODE << format("receiving UDP message failed: %s", strerror(errno)) << RESET_CODE
           << endl;
      continue;
    }
    ...
    if (msg.playerId == brain->config->playerId) {  // ignore self messages
      cout << YELLOW_CODE
           << format("discoveryID: %d, teamId:%d, playerId: %d, address: %s:%d",
                     msg.communicationId, msg.teamId, msg.playerId, inet_ntoa(addr.sin_addr),
                     ntohs(addr.sin_port))
           << RESET_CODE << endl;
      continue;
    } else {
      ...
      _teammate_addresses[addr.sin_addr.s_addr] = {addr.sin_addr.s_addr, msg.playerId, time_now};
    }
  }
}
...
void BrainCommunication::unicastCommunication() {
  while (_unicast_communication_flag) {
    ...
    for (auto it = _teammate_addresses.begin(); it != _teammate_addresses.end(); ++it) {
      ...
      int ret = sendto(_unicast_socket, &msg, sizeof(msg), 0, (sockaddr *)&_unicast_saddr,
                       sizeof(_unicast_saddr));
      if (ret < 0) {
        cout << RED_CODE << format("sendto failed: %s", strerror(errno)) << RESET_CODE << endl;
      }
    }
...
void BrainCommunication::clearupCommunicationUnicast() {
  ...
  if (_unicast_socket >= 0) {
    ...
    _unicast_socket = -1;
    cout << RED_CODE << format("Communication send socket has been closed.") << RESET_CODE << endl;
  }
...
void BrainCommunication::spinCommunicationReceiver() {
  ...
  while (_receive_communication_flag) {
    ssize_t len =
        recvfrom(_communication_recv_socket, &msg, sizeof(msg), 0, (sockaddr *)&addr, &addr_len);
