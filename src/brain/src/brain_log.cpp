...
void BrainLog::logStatics() {
  ...
  rerun::Collection<rerun::Vec2D> borders = {
      ...
      {-fieldDimensions.length / 2, fieldDimensions.width / 2},
      {-fieldDimensions.length / 2, -fieldDimensions.width / 2}};
  rerun::Collection<rerun::Vec2D> centerLine = {{0, -fieldDimensions.width / 2},
                                                {0, fieldDimensions.width / 2}};
  rerun::Collection<rerun::Vec2D> leftPenalty = {
      {-fieldDimensions.length / 2, fieldDimensions.penaltyAreaWidth / 2},
      {-(fieldDimensions.length / 2 - fieldDimensions.penaltyAreaLength),
       fieldDimensions.penaltyAreaWidth / 2},
      {-(fieldDimensions.length / 2 - fieldDimensions.penaltyAreaLength),
       -fieldDimensions.penaltyAreaWidth / 2},
      {-fieldDimensions.length / 2, -fieldDimensions.penaltyAreaWidth / 2}};
  ...
  rerun::Collection<rerun::Vec2D> leftGoal = {
      ...
      {-(fieldDimensions.length / 2 - fieldDimensions.goalAreaLength),
       fieldDimensions.goalAreaWidth / 2},
      {-(fieldDimensions.length / 2 - fieldDimensions.goalAreaLength),
       -fieldDimensions.goalAreaWidth / 2},
      {-fieldDimensions.length / 2, -fieldDimensions.goalAreaWidth / 2}};
  ...
  rerunLog.log("field/lines",
               rerun::LineStrips2D({borders, centerLine, leftPenalty, rightPenalty, leftGoal,
                                    rightGoal, rerun::Collection<rerun::Vec2D>(circle)})
  ...
  rerun::Collection<rerun::Vec2D> angle = {
      {3 * cos(0.2), 3 * sin(0.2)}, {0, 0}, {3 * cos(0.2), 3 * sin(-0.2)}};

  rerunLog.log("robotframe/lines", rerun::LineStrips2D({xaxis, yaxis, border2m, border1m, angle})
                                       .with_colors({0xFFFFFFFF})
