...
struct RoboCupGameControlData {
  ...
  uint16_t dropInTime;  // number of seconds passed since the last drop in. -1 (0xffff) before first
                        // dropin
  uint16_t secsRemaining;  // estimate of number of seconds remaining in the half
  uint16_t secondaryTime;  // number of seconds shown as secondary time (remaining ready, until free
                           // ball, etc)
  TeamInfo teams[2];
};
